    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 z-50 p-4 rounded-full shadow-2xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center ${
          isOpen ? 'bg-red-500 rotate-90' : 'bg-nebula-500 hover:bg-nebula-400'
        }`}
      >
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-24 right-6 w-96 max-w-[calc(100vw-3rem)] h-[600px] max-h-[70vh] bg-nebula-900/95 backdrop-blur-xl border border-nebula-700 rounded-3xl shadow-2xl z-40 flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${
            ...
            >
              <div className={`flex max-w-[85%] gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    msg.role === 'user' ? 'bg-nebula-700 text-gray-300' : 'bg-nebula-500 text-white'
                }`}>
